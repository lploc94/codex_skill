...
Use Claude Code's **native Agent tool** (built-in, no plugins needed) to spawn 4 parallel reviewers. Each uses the default general-purpose agent behavior with `run_in_background: true`.
